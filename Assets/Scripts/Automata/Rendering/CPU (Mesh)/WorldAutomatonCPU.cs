...
public class WorldAutomatonCPU : WorldAutomaton
{
    ...
    ArrayOfNativeList<float3x4> vertex_index_lists;
    ArrayOfNativeListWriter<float3x4> vertex_index_lists_writer;
    ...
    public override void Setup(WorldCellInfo[] cell_grid, int3[] block_grid)
    {
        ...
        this.world_quad_vertices = new NativeArray<float3x4>(this.automaton_size * 6, // 6 faces per voxel
            Allocator.Persistent,
        ...
        for(int i=0; i < this.element_materials.Length; i++)
        {
            ...
            this.element_materials[i] = (Material)Resources.Load(element_name);
        }
        ...
        {
            BuildVoxelWorldMeshMarchingCubes buildjob = new()
            {
    ...
    public override void RunMouseButtonBehaviors(Element state, int brush_size)
    {
        ...
        for (int nx = index.x - brush_size; nx <= index.x + brush_size; nx++)
        {
            for (int ny = index.y - brush_size; ny <= index.y + brush_size; ny++)
            {
                ...
                {
                    if (VoxelUtils.IsOutOfBounds(nx, ny, nz, GlobalConfig.world_automaton.automaton_dimensions)) continue;
                    VoxelAutomaton<Element>.SetCellNextState(this.cell_grid, GlobalConfig.world_automaton.automaton_dimensions, this.frame, nx, ny, nz, state);
                    int i = VoxelUtils.Index_FlatFromint3(nx, ny, nz, GlobalConfig.world_automaton.automaton_dimensions);
                    WorldCellInfo value = this.cell_grid[i];
                    this.cell_grid[i] = value;
                }
    ...
    public void RenderNextGridState()
    {
        ...
        vertex_index_lists = new();
        vertex_index_lists_writer = new();
        for (int i = 0; i < Elemental.number_of_elements; i++)
        {
            NativeList<float3x4> vertex_index_list = new(this.world_quad_vertices.Length, Allocator.TempJob);
            vertex_index_lists[i] = vertex_index_list;
            vertex_index_lists_writer[i] = vertex_index_list.AsParallelWriter();
        }

        // filter out the visible vertex indices for each element, so we don't have to create tons of meshes that only hold invisible vertices
        FilterVoxelVertexIndices jobVertexIndexFilter = new()
        {
            cell_grid = this.cell_grid,
            vertex_index_lists = vertex_index_lists_writer,
        ...
        {
            if (vertex_index_lists[i].Length != 0)
            {
                element_number_of_meshes = Mathf.CeilToInt(vertex_index_lists[i].Length * 4.0f / MAX_VERTICES_PER_MESH);
            }
            else
            {
        ...
        for (int i = 0; i < number_of_elements; i++)
        {
            ...
            int remaining_vertices = vertex_index_lists[i].Length * 4; // 4 vertex per quad
            ...
            for (int j = 0; j < element_number_of_meshes; j++)
            {
                ...
                tris_to_allocate = (vertices_to_allocate / 4) * 6;
        ...
        RenderNextGridParallel jobRender = new()
        {
            grid = this.cell_grid,
            element_mesh_data = vertex_index_lists,
            mesh_index_to_element = mesh_index_to_element_idx_and_element,
    ...
    private void OnApplicationQuit()
    {
        ...
        try
        {
            for (int i = 0; i < this.vertex_index_lists.Length; i++)
            {
                this.vertex_index_lists[i].Dispose();
            }
        }
        catch
        {
