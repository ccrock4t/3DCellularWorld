...
[BurstCompile]
public struct FilterVoxelVertexIndices : IJobParallelFor
{
    public Element filter_element;
    [NativeDisableParallelForRestriction]
    public NativeArray<WorldCellInfo> cell_grid;
    [WriteOnly]
    public ArrayOfNativeListWriter<float3x4> vertex_index_lists;
    [NativeDisableParallelForRestriction]
    ...
    public void Execute(int i)
    {
        ...
        int3 index = VoxelUtils.Index_int3FromFlat(voxel_flat_idx, this.automaton_dimensions);
        if (CanHideQuadToNeighbor(index, block_side)) return;
        NativeList<float3x4>.ParallelWriter vertex_index_render_list = vertex_index_lists[(int)state];
        vertex_index_render_list.AddNoResize(unfiltered_quad_vertices[i]);
    }
